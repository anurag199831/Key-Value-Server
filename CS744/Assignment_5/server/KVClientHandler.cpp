...
class KVClientHandler {
   ...
   public:
    ...
    pair<int, string> handle(int fd, string xml) {
        ...
        if (vec.size() == 3 and (vec.at(1) == "get" or vec.at(1) == "del")) {
            if (vec.at(2).length() > 256) {
                return make_pair(fd, formatter.getMessage("Oversized Key"));
            }
            Task task(fd, vec.at(1), vec.at(2));
            pool.addTaskToQueue(task);
            output = pool.getResults();
            cout << "Output: " << output.first << " " << output.second << endl;
            output.second =
                formatter.convertToXML(vec.at(1), vec.at(2), output.second);
        } else if (vec.size() == 4 and vec.at(1) == "put") {
            if (vec.at(2).length() > 256) {
                return make_pair(fd, formatter.getMessage("Oversized Key"));
            } else if (vec.at(3).length() > 256 * 1024) {
                return make_pair(fd, formatter.getMessage("Oversized Value"));
            }
            Task task(fd, vec.at(1), vec.at(2), vec.at(3));
            pool.addTaskToQueue(task);
            output.second =
                formatter.convertToXML(vec.at(1), vec.at(2), output.second);
        } else {
